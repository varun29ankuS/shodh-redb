...
use crate::bf_tree::fs::IoUringVfs;

#[cfg(feature = "std")]
use crate::bf_tree::fs::StdVfs;
...
use crate::bf_tree::{
    ...
    utils::{MappingTable, rw_lock::RwLock},
};
#[cfg(feature = "std")]
use std::path::Path;
...
impl LeafStorage {
    ...
    ) -> Result<Self, crate::bf_tree::error::IoErrorKind> {
    ...
    pub(crate) fn new(
        ...
        let vfs: Arc<dyn VfsImpl> = Arc::new(crate::bf_tree::fs::MemoryVfs::new());
        let page_table = PageTable::new(vfs.clone(), config.clone());
        let circular_buffer = CircularBuffer::new(
            config.cb_size_byte,
            config.cb_copy_on_access_ratio,
...
) -> Result<Arc<dyn VfsImpl>, crate::bf_tree::error::IoErrorKind> {
    match storage_backend {
        StorageBackend::Memory => Ok(Arc::new(MemoryVfs::new())),
        StorageBackend::Std => Ok(Arc::new(StdVfs::open(path.as_ref())?)),
        StorageBackend::StdWriteThrough => Ok(Arc::new(crate::bf_tree::fs::WriteThroughVfs::open(
            path.as_ref(),
        )?)),
        ...
        #[cfg(target_os = "linux")]
        StorageBackend::StdDirect => Ok(Arc::new(crate::bf_tree::fs::StdDirectVfs::open(
            path.as_ref(),
        )?)),
    }
}
...
mod tests {
    use crate::bf_tree::sync::{Arc, atomic::AtomicUsize};
