...
cfg_if! {
    if #[cfg(any(feature = "metrics-rt-debug-all", feature = "metrics-rt-debug-timer"))] {
        use crate::bf_tree::metric::{Timer, TlsRecorder, timer::{TimerRecorder, DebugTimerGuard}};
...
use crate::bf_tree::{
    ...
    mini_page_op::{LeafEntryXLocked, LeafOperations, ReadResult, upgrade_to_full_page},
    nodes::{
        CACHE_LINE_SIZE, DISK_PAGE_SIZE, INNER_NODE_SIZE, InnerNode, InnerNodeBuilder, LeafNode,
        MAX_KEY_LEN, MAX_LEAF_PAGE_SIZE, MAX_VALUE_LEN, PageID,
    ...
    utils::{Backoff, BfsVisitor, NodeInfo, get_rng, inner_lock::ReadGuard},
};
...
pub enum ScanIterError {
    ...
    IoError(crate::bf_tree::error::IoErrorKind),
}
...
impl BfTree {
    ...
    pub fn batch_insert_sorted_deferred_wal(
        &self,
        entries: &[(&[u8], &[u8])],
    ) -> Result<(), crate::bf_tree::BfTreeError> {
    ...
    pub fn flush_wal(&self) -> Result<(), crate::bf_tree::BfTreeError> {
        if let Some(wal) = &self.wal {
            wal.flush_and_wait().map_err(|_| {
                crate::bf_tree::BfTreeError::Io(crate::bf_tree::error::IoErrorKind::WalFlush)
            })?;
        }
        Ok(())
    }
...
pub(crate) fn key_value_physical_size(key: &[u8], value: &[u8]) -> usize {
    ...
    let meta_size = crate::bf_tree::nodes::KV_META_SIZE;
...
mod tests {
    use crate::bf_tree::BfTree;
    use crate::bf_tree::error::{BfTreeError, ConfigError};
    ...
    fn test_invalid_config_to_build_bf_tree() {
        ...
        let mut config = crate::bf_tree::Config::default();
        config.cb_min_record_size(4);
        ...
        config = crate::bf_tree::Config::default();
