...
use rand_08::{Error, RngCore, SeedableRng};
...
pub struct SmallThreadRng {
    rng: Rc<UnsafeCell<rand_08::rngs::SmallRng>>,
}
...
#[cfg(not(all(feature = "shuttle", test)))]
#[cfg(not(feature = "std"))]
pub(crate) fn get_rng() -> rand_08::rngs::SmallRng {
    use core::sync::atomic::{AtomicU32, Ordering};
    static SEED_CTR: AtomicU32 = AtomicU32::new(1);
    rand_08::rngs::SmallRng::seed_from_u64(SEED_CTR.fetch_add(1, Ordering::Relaxed) as u64)
}
...
pub(crate) enum NodeInfo {
    ...
    Inner {
        level: usize,
        ptr: *const crate::bf_tree::nodes::InnerNode,
    },
}
...
impl crate::bf_tree::fs::VfsImpl for TestVfs {
    fn read(
        &self,
        _offset: usize,
        _buf: &mut [u8],
    ) -> Result<(), crate::bf_tree::error::IoErrorKind> {
        Ok(())
    }

    fn write(&self, _offset: usize, _buf: &[u8]) -> Result<(), crate::bf_tree::error::IoErrorKind> {
        Ok(())
    }
    ...
    fn flush(&self) -> Result<(), crate::bf_tree::error::IoErrorKind> {
        Ok(())
    }
}
